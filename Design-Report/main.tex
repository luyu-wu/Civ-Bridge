...
Intuitively, we realized the box girder would be prone to distortional shear in a fashion that would collapse it to a parallelogram. We also saw this as a possible failure mode in papers \cite{heins_1978_box}. Realizing diaphragms should prevent this issue, we added them into the design. Note that this is separate from shear buckling, which also benefits from diaphragms. We also recognize that shear force is greatest at the supports, thus we dedicate 2 diaphragms near each support, much like Design 0 \cite{project_handout}.\\
