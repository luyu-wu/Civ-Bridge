...
\date{November 7, 2025 - CIV102 Bridge Project - TA: Christian Pavlidis}
...
\begin{minipage}{0.48\textwidth}
    \begin{equation}
        \sigma = \frac{My}{I}
        \label{Navier's Equation}
    \end{equation}
    \centering Navier's Equation
\end{minipage}
...
Starting from a HSS beam, our team diverged into the design space to consider better possible solutions during each iteration. For instance, different cross-sections were considered for better characteristics, diaphragms were added to control axial buckling, and the track was layered to provide more uniform moment. Shear force diagrams (SFDs), bending moment diagrams (BMDs), and their respective envelopes (BME, SFE) were computed for various train positions to predict applied stresses along the span. These stresses were then compared with material and buckling capacities to determine the factor of safety for each failure situation. After this, we optimimzed parameters such as web height, flange width, and track-depth to increase FOS across the span using provided course equations as a guide.

This report provides an overview of our team's design process throughout the project, from brainstorming to the final construction.
...
\begin{center}
    \textbf{Choose a 100 mm x 100 mm x 1.27 mm HSS as our starting point. }
\end{center}
...
of this project, we chose rectangular for the simplified geometry and load profiles. Since the track width must be at least 100 mm wide at any point, we allowed the HSS to be 100 mm x 100 mm x 1.27 mm. This is the most simple cross section,
...
We realized that since the upper half of the box girder is in compression while the bottom is in tension. Compression is a far less likely fail case, considering the materials properties of $\sigma_{tensile} = 30~MPa$ and $\sigma_{compression}=6~MPa$. Thus, it seemed more worth it to place the bottom paneling to the top. Doing this, we get a $\Pi$ shape of the bridge. However, we will keep bottom panels near the supports, since a bottom layer is required to rest the bridge on the supports. This will not be accounted for in our calculations, since this layer only exists near the supports. \\

We also realized that removing the bottom shifts the centroid up. This has 2 effects. First, it decreases the distance from the centroid, through Equation \ref{Navier's Equation} this decreases the stress. Second, it decreases the MOI as the distance of the deck is closer to the centroid.
...
\begin{center}
    \textbf{Set height to 80 mm and width to 120 mm, and move webs closer together.}
\end{center}
...
In Equation \ref{Navier's Equation}, $FOS \propto \sigma_{max}^{-1} \propto I \propto h^3$.
...
Similarly, $FOS \propto \sigma_{max}^{-1} \propto I \propto b$, from Equation \ref{Navier's Equation}, and $FOS \propto \sigma_{allowable} \propto b^{-2}$ from Equation \ref{Buckling of Compressive Flange}. This means the most efficient $b$ occurs when $\sigma_{buckle} = 6MPa$ for the flange. \\

Solving for $b$ and $h$ below, we get $h_{web} = 74\,\text{ mm, } b_{flange} = 121\,\text{ mm}$. Note that these values differ from the actual height and width of the bridge. For example, $h_{web} = h_{bridge} - 1.27(n+1)$, where n is the total number of top + bottom layers. Due to project constraints, the overall height of the bridge must be in multiples of 20 mm, thus we set h = 80 mm. Since we cannot cut the material accurately within 1 mm, we set b = 120 mm.
\begin{align*}
\sigma =& \frac{6\pi ^2E}{12(1-\mu^2)}\left(\frac{t}{h}\right)^2\\
6\text{ M{Pa}} =& 20562\text{ MPa}\times \left(\frac{1.27\text{ mm}}{h}\right)^2\\
h =& 74.3\text{ mm}
\end{align*}
\begin{align*}
\sigma =& \frac{4\pi ^2E}{12(1-\mu^2)}\left(\frac{t}{b}\right)^2\\
6\text{ M{Pa}} =& 13708\text{ MPa}\times \left(\frac{2\times1.27\text{ mm}}{b}\right)^2\\
b =& 121.0\text{ mm}
\end{align*}


An additional benefit for this low value of height is that shear buckling is unlikely to occur, proven by the equations below:
\begin{align*}
\tau &= \frac{5\pi^2E}{12(1-\mu^2)}\left[ \left(\frac{t}{h}\right)^2+ \left(\frac{t}{a}\right)^2 \right]\\
&\ge \frac{5\pi^2E}{12(1-\mu^2)} \left(\frac{t}{h}\right)^2\\
&\ge \frac{5}{6}\frac{6\pi^2E}{12(1-\mu^2)} \left(\frac{t}{h}\right)^2\\
&\ge \frac{5}{6}(6\text{ MPa}), \because\text{we set }\sigma_{buckle} \ge \sigma_{compression}\\
&> 4\text{ MPa}\\
&> \tau_{matboard}
\end{align*}

We also decided to move the webs closer together such that they are 100 mm apart (which is the minimum distance as required by the project constraints), without changing the top layer’s width. This does not change $I$, but increases the FOS against flange buckling.
...
Intuitively, we realized the box girder would be prone to distortional shear in a fashion that would collapse it to a parallelogram (Fig. \ref{fig:Distortial Shear Failure Mode}). We also saw this as a possible failure mode in papers \cite{heins_1978_box}. Realizing diaphragms should prevent this issue, we added them into the design. Note that this is separate from shear buckling, which also benefits from diaphragms. We also recognize that shear force is greatest at the supports, thus we dedicate 2 diaphragms near each support, much like Design 0 \cite{project_handout}.\\
...
Furthermore, verifications are done on the design for FOS against shear. Figure \ref{fig:python script for FOS} shows the Python script for computing the FOS against shear and Figure \ref{fig:handwirte for code} shows the algebra done by hand to derive certain lines of the code. The failure mode for shear is computed to be matboard material failure at the centroidal axis upon comparison with shear failure at glue. Thus, no extra modifications are required to prevent the design from failing from shear. \\ \\


\subsection{Design Iteration \#6\ - Finalizing All Dimensions}
\begin{center}
    \textbf{Calculated required layer lengths. Exact dimensions see Table \ref{fig:table for code output}.}
\end{center}
...
Figure \ref{fig:python script for each layer} shows the python script for computing the required length of each layer under a 1kN load, given cross-sectional geometry of the design and its BME. Table \ref{fig:table for code output} summarizes the output. \\ \\ \\

\subsection{Design Iteration \#7\ - Final Design - Adding the Bottom Back}
\begin{center}
    \textbf{Re-added the bottom layer and recalculated required layer lengths (Figure \ref{fig:Final diagram - cross-section} and \ref{fig:Final diagram - top layer}).}
\end{center}
...
In our previous calculations, we forgot to take into account failure by tension. Due to this, we added back the bottom layer. We ran the code in Figure \ref{fig:python script for each layer} again, with modified cross-sectional properties. Figure \ref{fig:layer code output} shows the output for the range unable to support for each number of extra layers under a 1kN load.
...
We then estimated this design’s failure load using a python script. Figure \ref{graph for all FOS} shows the graph of all of its FOS under failure load. The failure load is 1096N. The location with the least FOS is at x = 379 mm, with FOS = 1.009. \\


We then planned out how to cut the matboard. After allocating area to the webs, bottom layer and the full length top layer with a 5 mm tolerance for cutting, we decided to leave a greater length for each layer. Final product see Figure \ref{fig:Final diagram - top layer}. We also had 8 leftover diaphragms, which allows us to allocate them according to previously planned in design iteration \#5. Figure \ref{Final diaphragms} shows the diaphragms locations along the length of the bridge.


\begin{figure}[htb]
    \centering
    \includegraphics[width=\linewidth]{images/image2.png}
    \caption{ A graph of all FOS under the final design’s failure load, with 0 mm and 1200 mm at the two supports. For visual purposes, the FOS plotted is limited to at most 10. }
...
\begin{figure}[htb]
    ...
    \caption{The applied weight test is shown in this picture. In the highlighted part, you may observe the second issue (soffit uplift from web). \textit{9:00 PM Nov 20.}}
    \label{Testing}
\end{figure}
...
The second issue we observed was the lifting of the soffit (bottom deck). This is because of the height mismatch between the diaphragms and the web (estimated to be around 1 mm), where the soffit does not attach properly in certain areas near the diaphragms. This was addressed by adding glue tabs to hold the web to the soffit. This should have minimal effect on our calculations, as they assume this connection exists. \\
...
\begin{figure}[htb]
    ...
    \caption{Our Bridge. \textit{9:00 PM Nov 20.}}
    \label{Bridge}
\end{figure}
...
In summary, our bridge design went through numerous design phases, stringent construction, and ultimately, should theoretically be able to withhold the 1 kN load. \\
